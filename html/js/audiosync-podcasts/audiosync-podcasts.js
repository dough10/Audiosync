...
class AudioSyncPodcasts extends HTMLElement {
   ...
   * 
   * @example
   * document.querySelector('body').append(podcastLibrary);
   */
  connectedCallback() {
  ...
  _noShows() {
    ...
    this._container.append(noEpisodes);
  }
  ...
  async _addPodcastUI() {
    ...
    loaderElement.append(refreshIcon);
    ...
    addPodcastButton.append(fillButton('add', 'add'));
  ...
  async _createUnsubDialog(podcastTitleElement, url) {
    ...
    yesButton.append(fillButton('check', 'yes'));
    ...
    noButton.append(fillButton('close', 'no'));
    ...
    deleteConfirmationDialog.append(
    ...
    qs('body').append(deleteConfirmationDialog);
    await sleep(20);
   ...
   */
  async _updateEpisodeList(xmlURL, scrollEl) {
    // cache items marked as 'inlist'
    const inPlaylist = qsa('.episode[inlist]', scrollEl);

    try {
      // get podcast data
      const xmlString = await pywebview.api.xmlProxy(xmlURL);
  ...
  async _createEpisodeElement(title, episode, episodeList, xmlURL) {
    ...
    if ('itunes:episode' in episode) {
      ...
      ep_wrapper.append(ep_num);
    }

    episodeList.append(ep_wrapper);
    ...
    const buttonData = [
      {
        type: 'audiosync-small-button',
        icon: 'play',
        classes: [
          'ep-play'
        ],
        onClick: _ => this._playEpisode(ep_wrapper, fileStats, play_Object, episode)
      },
      {
        type: 'audiosync-small-button',
        icon: 'add',
        ...
        onClick: _ => this._addEpisodeToPlaylist(ep_wrapper, play_Object)
      }
    ];
    ...
    ep_wrapper.append(
      ...
      prog_bar,
      ep_title,
      ...buttonData.map(createButtonWithIcon)
    );

    if (!fileStats.exists) {
      // download button if file doesn't exist
      const download_button = createButtonWithIcon({
        type: 'audiosync-small-button',
        icon: 'download',
        classes: ['dl'],
        onClick: _ => this._downloadEpisode(
          title,
          episode,
          fileStats,
          xmlURL,
      ...
      ep_wrapper.append(download_button);
      return;
    }
    ep_wrapper.append(createButtonWithIcon({
      type: 'audiosync-small-button',
      icon: 'delete',
      classes: ['ep-del'],
      onClick: _ => this._deleteEpisode(ep_wrapper, fileStats, xmlURL, episodeList)
    }));
  }
  ...
  _lazyLoadOnScroll(title, episodes, scrollEl, xmlURL) {
    ...
    function load() {
      ...
      eps.forEach(episode => self._createEpisodeElement(title, episode, scrollEl, xmlURL)); 
    }
    load();
    scrollEl.onscroll = _ => {
  ...
  async _fetchAndParseXML(url) {
    ...
    const buttonData = [
      {
        ...
        icon: 'delete',
        classes: ['unsub'],
        onClick: _ => this._createUnsubDialog(podcastTitleElement, url)
      },
      {
        type: 'audiosync-small-button',
        icon: 'close',
    ...
    buttonContainer.append(...buttonData.map(createButtonWithIcon));
    ...
    this._container.append(podcastWrapper);
  }
}
